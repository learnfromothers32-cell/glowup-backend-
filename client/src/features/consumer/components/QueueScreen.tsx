...
const MOCK_BOOKINGS: Booking[] = [
  { bookingId: "bk-001", stylistName: "Marcus Reid", service: "Signature Fade + Beard Lineup", date: addDays(0), time: "14:30", status: "confirmed", queuePosition: 2, estimatedWaitMinutes: 18, salon: "Elite Cuts Studio", address: "12 Westfield Ave, Suite 3B", phone: "+233 538-281-749", price: 55, duration: 45, rating: 4.9, reviews: 128 },
  { bookingId: "bk-002", stylistName: "Aaliyah Chen", service: "Deep Condition & Blow-out", date: addDays(1), time: "10:00", status: "confirmed", queuePosition: 5, estimatedWaitMinutes: 42, salon: "Bloom Hair Atelier", address: "88 Park Row, 2nd Floor", phone: "+233 501-234-567", price: 75, duration: 60, rating: 4.8, reviews: 94 },
  { bookingId: "bk-003", stylistName: "Jerome Davis", service: "Classic Cut + Hot Towel Shave", date: addDays(5), time: "16:00", status: "confirmed", queuePosition: 1, estimatedWaitMinutes: 5, salon: "The Gentleman's Den", address: "204 North Blvd, Downtown", phone: "+233 244-567-890", price: 40, duration: 30, rating: 4.7, reviews: 211 },
  { bookingId: "bk-004", stylistName: "Sofia Okafor", service: "Knotless Box Braids", date: addDays(-2), time: "09:30", status: "cancelled", queuePosition: 3, estimatedWaitMinutes: 25, salon: "Roots & Crowns", address: "45 Vine Street", phone: "+233 555-321-654", price: 150, duration: 180, rating: 4.6, reviews: 57 },
];
...
function fmtEndTime(t: string, dur: number) {
  const [h, m] = t.split(":").map(Number);
...
function HeroCard({ booking, onView }: { booking: Booking; onView: () => void }) {
  const isToday = fmtDate(booking.date) === "Today";
  const isNext = booking.queuePosition === 1;
      ...
      <div className="p-5">
        ...
        <div className="flex gap-4">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-gray-500 mb-1">{booking.service}</p>
            <p className="text-xs text-gray-400">{booking.salon}</p>

            {/* Meta chips */}
            <div className="flex flex-wrap gap-1.5 mt-3">
              {[
                { icon: Calendar, text: fmtDate(booking.date) },
                { icon: Clock, text: fmtTime(booking.time) },
                { icon: Timer, text: `~${booking.estimatedWaitMinutes}m wait` },
              ].map(({ icon: Icon, text }) => (
          ...
          <div className="shrink-0 flex flex-col items-center gap-1">
            <QueueBadge position={booking.queuePosition} size="lg" />
            {isNext && (
...
// ─── Booking Row ──────────────────────────────────────────────────────────────
function BookingRow({ booking, onView, onCancel, index }: {
  booking: Booking; onView: () => void;
    ...
    >
      <div className="flex items-center gap-4 p-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 mt-1.5 text-[11px] text-gray-400">
            <span className="flex items-center gap-1">
              <Calendar size={10} />{fmtDate(booking.date)}
            </span>
            {booking.estimatedWaitMinutes > 0 && !isCancelled && (
              <span className={`flex items-center gap-1 font-medium ${
                booking.estimatedWaitMinutes <= 10 ? "text-green-600" : "text-amber-600"
              }`}>
                <Timer size={10} />~{booking.estimatedWaitMinutes}m
              </span>
            )}
          </div>
        </div>
        ...
        <div className="shrink-0 flex flex-col items-end gap-2">
          <QueueBadge position={booking.queuePosition} size="sm" />
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-semibold text-gray-900">{booking.stylistName}</h2>
        ...
        <div className="flex-1 overflow-y-auto">
            ...
            <div className="px-5 py-4 border-b border-gray-100">
              <div className={`p-4 rounded-xl ${isNext ? "bg-green-50 border border-green-100" : "bg-amber-50 border border-amber-100"}`}>
                ...
                <div className="h-2 bg-black/5 rounded-full overflow-hidden">
                  <motion.div
                    initial={{ width: 0 }}
                    animate={{ width: `${Math.max(15, 100 - (booking.queuePosition - 1) * 15)}%` }}
                ...
                <p className={`text-xs mt-2 ${isNext ? "text-green-600" : "text-amber-600"}`}>
                  {isNext
                    ...
                    : `${booking.queuePosition - 1} ${booking.queuePosition - 1 === 1 ? "person" : "people"} ahead of you`}
                </p>
              </div>
            </div>
          )}
          ...
          <div className="px-5 py-3">
            {[
              { label: "Salon", value: booking.salon },
              { label: "Date", value: fmtDateFull(booking.date) },
              { label: "Time", value: `${fmtTime(booking.time)} – ${fmtEndTime(booking.time, booking.duration)}` },
              { label: "Duration", value: `${booking.duration} min` },
              { label: "Price", value: `$${booking.price}.00` },
          ...
          <div className="px-5 py-3 border-t border-gray-100">
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-0.5">
                {[1, 2, 3, 4, 5].map(s => (
                  <Star key={s} size={14} className={s <= Math.round(booking.rating) ? "text-amber-400" : "text-gray-200"} fill={s <= Math.round(booking.rating) ? "#f59e0b" : "none"} />
                ))}
              </div>
              <span className="text-sm font-bold text-gray-900">{booking.rating}</span>
              <span className="text-xs text-gray-400">({booking.reviews} reviews)</span>
            </div>
          </div>
          ...
          <div className="px-5 py-3 border-t border-gray-100">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs font-mono text-gray-600">{booking.bookingId}</p>
              </div>
    ...
    >
      ...
      >
        ...
        <div className="p-5">
          ...
          <p className="text-sm text-gray-600 mb-1">{booking.stylistName}</p>
          <p className="text-xs text-gray-400 mb-3">{booking.service} · {fmtDate(booking.date)} at {fmtTime(booking.time)}</p>

          <div className="flex items-start gap-2 p-3 rounded-xl bg-amber-50 border border-amber-100 mb-5">
            <AlertTriangle size={13} className="text-amber-500 shrink-0 mt-0.5" />
            <p className="text-xs text-amber-700 leading-relaxed">
              Cancellations within 24 hours may incur a fee of ${Math.round(booking.price * 0.5)}.
            </p>
          </div>
...
export default function QueueScreen() {
  ...
  const filtered = useMemo(() => {
    let list = [...bookings];
    if (filter === "today") list = list.filter(b => b.status === "confirmed" && b.date === today);
    else if (filter === "upcoming") list = list.filter(b => b.status === "confirmed" && b.date >= today);
    else if (filter === "cancelled") list = list.filter(b => b.status === "cancelled");

    if (search.trim()) {
      const q = search.toLowerCase();
      list = list.filter(b => b.stylistName.toLowerCase().includes(q) || b.service.toLowerCase().includes(q) || b.salon.toLowerCase().includes(q));
    }
  ...
  const heroBooking = upcoming.find(b => b.date === today) ?? upcoming[0];

  // ── Cancel handler ──
  const handleCancel = useCallback(async () => {
    if (!cancelTarget) return;
    setCancellingId(cancelTarget.bookingId);
    await new Promise(r => setTimeout(r, 1100));
    setBookings(prev => prev.map(b => b.bookingId === cancelTarget.bookingId ? { ...b, status: "cancelled" as const } : b));
        ...
        {loading ? (
          ...
          <>
            ...
            {heroBooking && filter !== "cancelled" && (
              <HeroCard booking={heroBooking} onView={() => setDetailBooking(heroBooking)} />
            )}
      ...
      <AnimatePresence>
        {cancelTarget && (
          <CancelModal key="cancel" booking={cancelTarget}
            ...
            loading={cancellingId === cancelTarget.bookingId}
          />
        )}
      </AnimatePresence>
