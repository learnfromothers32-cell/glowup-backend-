    ...
    >
      <div className="flex items-center gap-4 p-4">
        <div className="shrink-0 w-16 text-center">
          <p className="text-sm font-bold text-text-primary dark:text-text-dark-primary tabular-nums">{fmtTime(fmtISO(booking.startTime)).split(" ")[0]}</p>
          <p className="text-[10px] text-text-muted dark:text-text-dark-muted font-medium">{fmtTime(fmtISO(booking.startTime)).split(" ")[1]}</p>
        </div>

        <div className={`w-px h-10 shrink-0 ${isCancelled ? "bg-red-200 dark:bg-red-900/40" : fmtDate(new Date(booking.startTime)) === "Today" ? "bg-green-300 dark:bg-green-700/50" : "bg-gray-200 dark:bg-gray-600"}`} />

        <div className="shrink-0">
          {stylistImage ? (
            <img src={stylistImage} alt={stylistName} className={`w-11 h-11 rounded-xl object-cover ring-1 ring-gray-100 dark:ring-gray-700/40 ${isCancelled ? "grayscale" : ""}`} />
          ) : (
            <div className="w-11 h-11 rounded-xl bg-gray-100 dark:bg-surface-dark-tertiary flex items-center justify-center">
              <span className="text-xs font-bold text-text-muted dark:text-text-dark-muted">{initials(stylistName)}</span>
            </div>
          )}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-text-secondary dark:text-text-dark-secondary truncate">{serviceName}</p>
          <div className="flex items-center gap-3 mt-1.5 text-[11px] text-text-muted dark:text-text-dark-muted">
        ...
        <div className="shrink-0 flex flex-col items-end gap-2">
          {booking.totalPrice > 0 && (
            <p className={`text-sm font-bold ${isCancelled ? "text-text-muted dark:text-text-dark-muted" : "text-text-primary dark:text-text-dark-primary"}`}>${booking.totalPrice}</p>
          )}
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-surface-dark-tertiary">
      <div className="max-w-3xl mx-auto px-4 pb-20">
        <div className="pt-14 pb-6">
          <h1 className="text-2xl font-bold text-text-primary dark:text-text-dark-primary tracking-tight mb-1">My Bookings</h1>
          <p className="text-sm text-text-muted dark:text-text-dark-muted">Manage your appointments and schedule</p>
        </div>
        ...
        {isLoading ? (
          <div className="space-y-3">
            {[1, 2, 3, 4].map((i) => <div key={i} className="h-20 rounded-2xl bg-white dark:bg-surface-dark-secondary border border-gray-100 dark:border-gray-700/40 skeleton-pulse" style={{ animationDelay: `${i * 0.1}s` }} />)}
          </div>
        ) : filtered.length === 0 ? (
              ...
              {grouped.map(([dateLabel, items]) => (
                <div key={dateLabel} className="mb-6">
                  <div className="flex items-center gap-3 mb-3">
                    <h3 className="text-xs font-bold uppercase tracking-wider text-text-muted dark:text-text-dark-muted">{dateLabel}</h3>
                    <div className="flex-1 h-px bg-gray-100 dark:bg-gray-700/40" />
                    <span className="text-[10px] font-bold text-text-muted dark:text-text-dark-muted">{items.length}</span>
                  </div>
